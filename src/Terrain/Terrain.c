...
#ifdef __3DS__
	gTerrainTextureDetail = SUPERTILE_DETAIL_3DS;
#else
...
static void	ShrinkSuperTileTextureMap(const uint32_t *srcPtr, uint32_t *dstPtr)
{
	_Static_assert(SUPERTILE_TEXSIZE_SHRUNK == 64, "rewrite this for new supertile texmap size!");
	_Static_assert(SUPERTILE_SIZE * OREOMAP_TILE_SIZE == 160, "rewrite this for new oreomap tile size!");

	for (int y = 0; y < SUPERTILE_TEXSIZE_SHRUNK; y++)
	{
		for (int x = 0; x < SUPERTILE_TEXSIZE_SHRUNK; x += 2) // shrink a block of 5 pixels from src into 2 pixels in dst
		{
			uint32_t c1 = srcPtr[1];						  // get colors to average
			uint32_t c2 = srcPtr[2];
			uint32_t c3 = srcPtr[3];
			uint32_t c4 = srcPtr[4];
			uint32_t r = (((c1 >> 24) & 0xff) + ((c2 >> 24) & 0xff) + ((c3 >> 24) & 0xff) + ((c4 >> 24) & 0xff)) >> 2;
			uint32_t g = (((c1 >> 16) & 0xff) + ((c2 >> 16) & 0xff) + ((c3 >> 16) & 0xff) + ((c4 >> 16) & 0xff)) >> 2;
			uint32_t b = (((c1 >> 8)  & 0xff) + ((c2 >> 8)  & 0xff) + ((c3 >> 8)  & 0xff) + ((c4 >> 8)  & 0xff)) >> 2;
			uint32_t a = ((c1 & 0xff) + (c2 & 0xff) + (c3 & 0xff) + (c4 & 0xff)) >> 2;

			dstPtr[0] = srcPtr[0];							// save #0
			dstPtr[1] = ((r & 0xff) << 24) | ((g & 0xff) << 16) | ((b & 0xff) << 8) | (a & 0xff);
			dstPtr += 2;
			srcPtr += 5;
		}

		if (y % 2 == 1)						// skip 3/5 lines
			srcPtr +=  3 * SUPERTILE_SIZE * OREOMAP_TILE_SIZE;
	}
}
